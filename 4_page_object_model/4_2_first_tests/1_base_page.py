...
# Давайте перепишем тест из файла test_main_page.py с помощью паттерна Page Object,
# который мы добавили на этапе подготовки окружения.
...
# как они могут облегчить нам жизнь. Сейчас самая главная задача — осознать принципы работы.
...
# class BasePage():
# 4. Теперь в наш класс нужно добавить методы.
...
# def open(self):
# и реализуйте этот метод: нужна всего одна строка.
# Эту строку нужно отправить в качестве ответа на это задание, без отступов.

# 6. После того как Stepik принял ваш ответ как правильный,
# добавьте новые файлы в Git и зафиксируйте изменения коммитом (не забудьте осмысленное сообщение).
