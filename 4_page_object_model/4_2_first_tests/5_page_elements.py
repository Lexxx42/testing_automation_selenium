...
# Представим себе ситуацию, что у нас модный быстрый agile: разработчики постоянно вносят изменения в продукт.
# В какой-то прекрасный момент изменения коснулись и шапки сайта.
# Вот приходит к вам разработчик с новой ссылкой и говорит протестировать.
...
# Тесты упали, и теперь нам нужно их поддерживать, то есть чинить.
# Подберите новый селектор к ссылке на логин.

# Нам придется поправить в файле main_page.py несколько мест, где используется измененный селектор.
# Посчитайте, сколько строк вам нужно будет отредактировать,
# чтобы починить ваши тесты, и внесите полученное число в первое поле ответа ниже.

# Чтобы этого избежать,
# при проектировании тестов (да и вообще кода)
...
# Они, конечно, снова упадут.
# Но теперь посчитайте, сколько строк вам нужно будет отредактировать,
...
# Исправлять руками сломанные селекторы во всем проекте — долго и муторно,
# и есть большой риск забыть и оставить старый селектор.
...
# А ещё спринт спустя промоакция закончилась,
# и фичу с изменением шапки откатили назад.
# Теперь ссылка работает так же, как раньше.
# Удалите ссылку с промоакцией, и верните обычную ссылку для запуска тестов:

# link = "http://selenium1py.pythonanywhere.com/"

# Не забудьте вернуть старый селектор #login_link, так чтобы тесты снова проходили.
