...
# Чтобы выводить адекватное сообщение об ошибке,
# мы будем все проверки осуществлять с помощью assert и перехватывать исключения.

# Для этого напишем вспомогательный метод поиска элемента в нашей базовой странице BasePage,
# который будет возвращать нам True или False.
...
# 2. Теперь в этом же классе реализуем метод is_element_present,
...
# Отлично! Теперь для всех проверок,
# что элемент действительно присутствует на странице,
# мы можем использовать этот метод.

# 3. Теперь модифицируем метод проверки ссылки на логин так,
# чтобы он выдавал адекватное сообщение об ошибке:
