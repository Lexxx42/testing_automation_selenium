...
# Эта задача для настоящих ниндзя автотестинга.
# Не потому что она сложная,
# а потому что сейчас мы будем ловить с вами настоящий баг с помощью наших автотестов.
# Для нашего интернет-магазина было запущено несколько новых промо-акций,
# одна из которых привела к появлению бага.
...
# Мы просто запустим всё тот же тест на странице
# http://selenium1py.pythonanywhere.com/catalogue/coders-at-work_207/ с параметризацией.
# Вам нужно определить, при каком значении параметра promo автотест упадет.
...
# лучше всего пометить падающий тест как xfail или skip.
# Помните, как мы такое проворачивали в третьем модуле?

# Освежить память: XFail: помечать тест как ожидаемо падающий.
...
# https://pytest.org/en/stable/how-to/skipping.html#skip-xfail-with-parametrize
