...
# ВАЖНО!
# Вообще говоря манипулировать браузером в сетапе
# и уж тем более что-то там проверять — это плохая практика,
# лучше так не делать без особой необходимости.
# Здесь этот пример исключительно в учебных целях,
# чтобы вы попробовали писать сетапы для тестов.
# В реальной жизни мы реализовали бы все эти манипуляции
...
# но и для зарегистрированных пользователей.
# Для этого:

# В файле test_product_page.py добавьте новый класс для тестов TestUserAddToBasketFromProductPage.
# Добавьте туда уже написанные тесты test_guest_cant_see_success_message
# и test_guest_can_add_product_to_basket и переименуйте, заменив guest на user.
...
# yield писать не нужно — пользователей удалять мы не умеем.
# Генерировать email адреса для пользователей можно по-разному,
# один из вариантов, чтобы избежать повторения,
