...
# В работе с веб-страницами не всегда получается найти селектор,
# однозначно описывающий путь к нужному элементу.
# В такой ситуации для тестировщика лучшим решением проблемы будет пойти
# к фронтенд-разработчику проекта и договориться о специальном атрибуте,
# который будет использоваться в автотестах.
# Таким образом можно повысить тестируемость приложения.
# Увы, проекты бывают разные, и не всегда это возможно.
# И когда другого выхода больше нет, а автоматизировать как-то надо,
# можно обратиться к помощи языка запросов XPath.

# На тему XPath мнения расходятся, но, как бы то ни было,
# это мощный и гибкий инструмент, который позволяет писать сложные запросы для поиска элементов.

# Прежде всего, XPath (XML Path Language) это язык запросов,
# который использует древовидную структуру документа.
...
# и давайте на её примере разберемся в основах синтаксиса.
# Попробуйте вбить каждый из запросов-примеров в строку поиска,
...
# В этом случае, для поиска того же хедера,
...
# Да, XPath — это единственный способ найти элемент по внутреннему тексту.
# Если мы хотим найти блок текста с котом-Лениным,
...
# Для этого нужна функция contains.
...
# это удобно, если у элемента несколько классов.
...
# 1. Не используйте селекторы вида //div[1]/div[2]/div[3]
# без крайней нужды: по такому селектору невозможно с первого раза понять,
# что за элемент вы ищете.
# А когда структура страницы хоть немного изменится,
# то ваш селектор с большой вероятностью перестанет работать;

# 2. Если есть возможность использовать CSS-селекторы:
# сlass, id или name — лучше использовать их вместо поиска по XPath;
...
# 8. Не стоит использовать разные расширения для браузеров по поиску XPath:
...
# В курсе мы не будем работать с XPath-селекторами,
# и в основном будем использовать CSS.
