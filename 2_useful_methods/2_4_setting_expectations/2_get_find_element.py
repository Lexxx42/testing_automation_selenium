...
# затем находим нужную кнопку с помощью одного из методов
# find_element_by_ и нажимаем на нее с помощью метода click.
# Далее находим новый элемент с текстом
...
# во втором случае автотест упадет с сообщением
# NoSuchElementException для элемента c id="verify".
...
# Но тест будет работать абсолютно стабильно,
...
# Если страница интерактивная, то браузер будет считать,
# что страница загружена, при этом продолжат выполняться загруженные браузером скрипты.
...
# button = browser.find_element(By.ID, "verify") элемент с id="verify" еще не отображается на странице.
