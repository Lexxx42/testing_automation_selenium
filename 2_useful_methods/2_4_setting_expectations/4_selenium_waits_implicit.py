...
# Надеемся, вы поняли, что решение с
# time.sleep() плохое: оно не масштабируемое и трудно поддерживаемое.

# Идеальное решение могло бы быть таким:
# нам всё равно надо избежать ложного падения тестов
# из-за асинхронной работы скриптов или задержек от сервера,
# поэтому мы будем ждать появление элемента на странице
...
# чтобы применить его ко всем тестам.

# Ожидание называется неявным (Implicit wait),
# так как его не надо явно указывать каждый раз,
# когда мы выполняем поиск элементов,
...
# Теперь мы можем быть уверены, что при небольших задержках в работе сайта
# наши тесты продолжат работать стабильно.
# На каждый вызов команды find_element WebDriver
# будет ждать 5 секунд до появления элемента на странице прежде,
