...
# Давайте теперь рассмотрим реальную ситуацию,
# когда пользователь должен кликнуть на элемент,
# который внезапно оказывается перекрыт другим элементом на странице.

# Для клика в WebDriver мы используем метод click().
# Если элемент оказывается перекрыт другим элементом,
# то наша программа вызовет следующую ошибку:

# selenium.common.exceptions.WebDriverException:
# Message: unknown error:
# Element <button type="submit" class="btn btn-default" style="margin-bottom: 1000px;">...</button>
# is not clickable at point (87, 420).
...
button.click()
"""
# Теперь вы можете сами посмотреть на эту страницу и увидеть,
# что огромный футер действительно перекрывает нужную нам кнопку.
# Футером (footer) называется нижний блок, который обычно одинаков для всех страниц сайта.
# Чтобы понять, как решить эту проблему, нужно разобраться, как работает метод click().

# В первую очередь WebDriver проверит,
# что ширина и высота элемента больше 0, чтобы по нему можно было кликнуть.

# Затем, если элемент находится за границей окна браузера,
# WebDriver автоматически проскроллит страницу,
# чтобы элемент попал в область видимости,
...
# Selenium рассчитывает координаты центра элемента и производит клик в вычисленную точку.
...
# Если мы столкнулись с такой ситуацией,
# мы можем заставить браузер дополнительно проскроллить нужный элемент,
# чтобы он точно стал видимым.
...
# В итоге, чтобы кликнуть на перекрытую кнопку,
# нам нужно выполнить следующие команды в коде:
...
# !Важно.
# Мы не будем в этом курсе изучать, как работает JavaScript,
...
# Для этого откройте инструменты разработчика в браузере,
# перейдите на вкладку консоль (console),
# скопируйте туда этот код и нажмите Enter.
# Таким образом можно протестировать кусочки js кода прежде чем внедрять его в свои тесты на python.

# Обратите внимание,
# что в коде в WebDriver нужно использовать ключевое слово return.
# Также его нужно будет использовать,
# когда вы захотите получить какие-то данные после выполнения скрипта.
