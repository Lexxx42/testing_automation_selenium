...
# Если нам понадобится загрузить файл на веб-странице,
# мы можем использовать уже знакомый нам метод send_keys.
# Только теперь нам нужно в качестве аргумента передать путь к нужному файлу
# на диске вместо простого текста.

# Чтобы указать путь к файлу,
# можно использовать стандартный модуль Python для работы с операционной системой — os.
...
# Обратите внимание, что это будет работать только при запуске кода из файла,
# в интерпретаторе не сработает.

# Если совсем непонятно что происходит, пример:

# Допустим, мы написали код скрипта и сохранили код в
# lesson2_step7.py в своей локальной папке D:\stepik_homework.
# Активируем виртуальное окружение и запускаем его python lesson2_step7.py.
# В таком случае конструкция os.path.abspath(os.path.dirname(__file__))
# вернет нам путь до директории файла с кодом,
# то есть D:\stepik_homework.
