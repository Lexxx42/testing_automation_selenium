...
# Для поиска Selenium предоставляет метод find_element,
...
# то вам повезло, и вы чаще всего будет использовать этот метод,
...
# Если страница у вас загрузилась, но дальше ничего не происходит,
# вернитесь обратно в консоль, в которой вы запускали ваш скрипт.
# Скорее всего, вы увидите там ошибку NoSuchElementException.
# Она будет выглядеть следующим образом:

# selenium.common.exceptions.NoSuchElementException:
# Message: no such element: Unable to locate element: {"method":"id","selector":"submit"}
...
# подходящих под заданные вами параметры поиска.
# В этом случае WebDriver вернет вам только первый элемент,
# который встретит во время поиска по HTML.
# Если вам нужен не первый, а второй или следующие элементы,
# вам нужно либо задать более точный селектор для поиска,
# либо использовать методы find_elements, которые мы рассмотрим чуть позже.

# Иногда в статьях про Selenium WebDriver вы также будете встречать термин "локаторы",
# под которым подразумеваются стратегии поиска и значения,
# по которым должен выполняться поиск.
