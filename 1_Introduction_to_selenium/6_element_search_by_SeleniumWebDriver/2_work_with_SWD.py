...
# Если вы уже пробовали запускать примеры скриптов,
# то могли заметить, что браузер не всегда закрывается после выполнения кода.
...
# Чтобы не остаться без оперативной памяти после запуска нескольких скриптов,
...
# или открыл что-то в новом окне или вкладке браузера,
...
# Но что будет, если скрипт не дойдет до выполнения этого финального шага,
# а упадет с ошибкой где-то раньше?

# Для того чтобы гарантировать закрытие,
# даже если произошла ошибка в предыдущих строках,
...
# Подробно говорить об обработке исключений мы сейчас не будем,
# здесь важно понимать только то,
# что даже если в коде внутри блока try произойдет какая-то ошибка,
# то код внутри блока finally выполнится в любом случае.
# Советуем добавлять такую обработку ко всем своим скриптам при
# выполнении задач этого и следующего модулей,
