...
# ля этого воспользуемся методом find_element_by_link_text:

# link = browser.find_element(By.LINK_TEXT, text)

# В качестве аргумента в метод передается такой текст,
# ссылку с которым мы хотим найти.
# Это тот самый текст, который содержится между открывающим
# и закрывающим тегом <a> вот тут </a>
...
# а можно выполнить в интерпретаторе,
