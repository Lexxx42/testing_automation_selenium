...
# Если код написан правильно и работал ранее,
# то такой результат равносилен тому,
...
# В этом примере мы использовали метод time.sleep(1),
# чтобы дождаться загрузки следующей страницы, прежде чем выполнять проверки.
# Если вы будете запускать код без этого метода, ваш код может внезапно упасть,
# хотя проходил ранее.
# Без использования такой паузы WebDriver может перейти к поиску тега h1 слишком рано,
# когда новая страница еще не загрузилась.
...
# Метод time.sleep(1) говорит Python подождать 1 секунду,
# прежде чем выполнять следующую строчку кода.
...
# Еще одним фактором, влияющим на стабильность работы тестов,
# является принцип асинхронности выполнения кода JavaScript.
...
# когда они не проходят по причине нестабильной скорости интернета или других причин,
