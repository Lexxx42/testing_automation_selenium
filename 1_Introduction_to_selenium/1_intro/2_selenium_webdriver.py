...
# Эти две части нужно как-то связать воедино. И как раз для этого нужен Selenium WebDriver.
...
# Однако, если вы будете искать в интернете какую-то информацию, связанную с Selenium WebDriver,
# то скорее всего увидите, что используется более общий термин Selenium.
# Вам нужно будет смотреть на контекст, чтобы определить, какая часть проекта Selenium имеется в виду.
...
# разработчики браузеров самостоятельно реализуют "драйвер" – программу,
# которая позволяет интерфейсу Selenium манипулировать кнопками, текстами, элементами и т.д.


# В этом курсе мы будем работать со следующим набором инструментов:
# pic2

# Интересный факт! WebDriver стал настолько популярен,
# что его признали стандартом протокола взаимодействия с браузером.

# Подробнее можно почитать посты Алексея Баранцева: они хоть и старые, все еще хорошие.
# Он сейчас – один из тех разработчиков, кто вносит вклад в проект Selenium.
