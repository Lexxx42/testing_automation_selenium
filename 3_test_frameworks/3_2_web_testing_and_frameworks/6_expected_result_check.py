...
# которая проверяет истинность утверждений.
# assert True не приводит к выводу дополнительных сообщений,
# а вот assert False вызовет исключение AssertionError.

# Рассмотрим работу assert на примере встроенной функции abs(),
# которая возвращает абсолютное значение числа по модулю.
# Для этого активируйте созданное ранее виртуальное окружение
# и запустите интерпретатор Python.
...
# то в консоли выводится лог ошибки с названием файла и номером строчки,
# в которой произошла ошибка, а также тип ошибки
# AssertionError:
...
# Для добавления дополнительного сообщения можно при вызове assert
# через запятую написать нужное сообщение,
