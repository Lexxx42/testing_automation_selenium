...
# Когда тестов становится много,
# сложно становится запускать только тесты из нужных тест-сьютов.

# Для каждого теста нужно создавать тестовые данные и окружение отдельно.

# Например, если мы захотим для каждого теста запускать браузер,
# а после завершения теста браузер закрывать,
# то логику работы с браузером придется дублировать в коде каждого теста.

# Если один из тестов завершится с ошибкой, например,
# тест упадёт с ошибкой AssertionError,
# то последующие тесты не запустятся.
# Мы не узнаем, были ли проблемы в этих тестах,
...
# Можно выделить три основных тестовых фреймворка для Python:
# unittest, PyTest и nose.

# Модуль unittest является встроенным инструментом Python — и это его большой плюс.
# PyTest и nose устанавливаются дополнительно,
# они позволяют получить расширенные возможности по сравнению с unittest.
# Мы кратко рассмотрим, как используется unittest, а затем изучим возможности PyTest,
# который позволяет писать более простой код тестов по сравнению с unittest
# и гибко настраивать запуск тестов.
# Еще один плюс использования PyTest в том,
# что для него существует большое количество плагинов,
