...
# Давайте теперь изменим наши предыдущие тесты,
# чтобы их можно было запустить с помощью unittest.
# Для этого нам понадобится выполнить следующие шаги:

# Импортировать unittest в файл: import unittest
# Создать класс, который должен наследоваться от класса
# TestCase: class TestAbs(unittest.TestCase):
# Превратить тестовые функции в методы,
# добавив ссылку на экземпляр класса self в качестве первого аргумента функции:
# def test_abs1(self):
...
# было запущено два теста, один тест выполнился с ошибкой.
...
# Если вы хотите использовать unittest в своих проектах, вы можете изучить документацию самостоятельно.
