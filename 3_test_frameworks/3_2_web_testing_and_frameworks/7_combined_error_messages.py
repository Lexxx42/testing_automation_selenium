...
# которые показываются при падении теста.
# Почему это важно?
# Хорошо написанный текст помогает быстро локализовать найденный баг и разобраться в том,
# что произошло и из-за чего тест упал.
# Хороший assert сэкономит вам часы вашей работы,
# особенно когда количество тестов переходит за сотню.

# В целом, тут как с любым фидбеком: важно давать его точно и актуально.
# Если вы проверяете наличие элемента, то обязательно пишите,
# что это за элемент по смыслу на странице:
...
# Если вы работаете с каким-то текстом
# (например, проверяете информационное сообщение, текущий url,
# ссылку, placeholder в input-элементе или любой другой текст),
# в сообщении об ошибке всегда лучше выводить оба значения:
# то, которое ожидалось, и то, которое получили по факту.
...
# который появился в Python3.6, носит название f-strings.
# Он позволяет исполнять выражения на Python прямо внутри строк,
# обладает еще большей лаконичностью и удобством использования.
# Для использования возможностей f-strings нужно указывать символ f перед строкой в таком формате:
# f"ваша строка {my_var}".
# В фигурных скобках указывается имя переменной,
# значение которой надо подставить в строку,
...
# Дважды считывать атрибут — это плохая практика,
# потому что при повторном считывании текст на странице может измениться,
# и вы получите неактуальный текст об ошибке.
