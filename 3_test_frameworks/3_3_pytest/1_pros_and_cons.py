...
# Для написания данного урока мы вдохновлялись статьёй на Хабре,
...
# 3) PyTest не требует написания дополнительных специфических конструкций в тестах,
# как того требует unittest (no boilerplate).

# 4) Для проверок используется стандартный assert из Python.

# 5) Возможность создания динамических фикстур
# (специальных функций, которые настраивают тестовые окружения и готовят тестовые данные).
...
# 8) Наличие маркировок (marks),
# которые позволяют маркировать тесты для их выборочного запуска.

# 9) Возможность передавать дополнительные параметры через
# командную строку для настройки тестовых окружений.

# 10) Большое количество плагинов,
# которые расширяют возможности PyTest и позволяют решать узкоспециализированные проблемы,
# что может сэкономить много времени.

# Рассмотрим минусы PyTest:

# 1) PyTest требуется устанавливать дополнительно,
# так как он не входит в стандартный пакет библиотек Python, в отличие от unittest.
# Нужно не забывать об этом,
# когда вы будете настраивать автоматический запуск тестов с помощью CI-сервера.

# 2) Использование PyTest требует более глубокого понимания языка Python,
# чтобы разобраться, как применять фикстуры, параметризацию и другие возможности PyTest.
