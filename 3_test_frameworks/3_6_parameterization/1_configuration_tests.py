...
# Ранее мы добавили фикстуру browser,
# которая создает нам экземпляр браузера для тестов в данном файле.

# Когда файлов с тестами становится больше одного,
# приходится в каждом файле с тестами описывать данную фикстуру.
# Это очень неудобно.
...
# о которой вы обязательно должны знать.
# PyTest автоматически находит и подгружает файлы conftest.py,
# которые находятся в директории с тестами.
# Если вы храните все свои скрипты для курса в одной директории,
# будьте аккуратны и следите, чтобы не возникало ситуации,
...
# В таком случае применяются ОБА файла conftest.py,
# что может вести к непредсказуемым ошибкам и конфликтам.

# Таким образом можно переопределять разные фикстуры,
# но мы в рамках курса рекомендуем придерживаться одного файла
...
# чтобы не было разных conftest во вложенных друг в друга директориях,
